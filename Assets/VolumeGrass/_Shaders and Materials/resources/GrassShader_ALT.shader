...
//
// openGl GLSL SM3.0 (compatible with all SM3.0 cards on Mac), works also on d3d9
// does not use tex2Dlod in conditionals (and early function exit) as this is totally unreliable inside fragment programs on Mac and could just crash Mac player
// this shader is not optimized for early break surface program/loop so it works much slower in most cases
//
Shader "Grass Shader ALT" {
	Properties {
		//
	...
	//
	//
	// subshader for OpenGL, SM3.0
	// no quick exit optimization available here (must execute full code, so even at far distances everything is executed)
	// so DEBUG_PERFORMANCE and TRANSPARENCY_BREAK_VALUE functionality _unavailable_ here (comparing to d3d9 SubShader)
	//
	SubShader {
		...
		// function(hash_noise_u) should give possibly random value
		...
		//        1 MUL+1 SUM on color per iteration
		//
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			half3 base_col=tex2D(_MainTex, IN.uv_MainTex*_fardistancetiling).rgb;
		 	...
		 	zoffset = (zoffset>1) ? 1 : zoffset;
				...
				float bottom_cut=_bottom_coloring_value.a*4*(1-zoffset*_bottom_coloring_border_damp);
					...
					bottom_cut*=tex2D(NOISE_BCOLORING_TEX, IN.uv_MainTex*_bottom_coloring_noise_tiling.xy+_bottom_coloring_noise_tiling.zw).NOISE_CHANNEL_BCOLORING;
				#endif
			...
			sceneUVZ.z = 1.0 / ((_ZBufferParamA  * tex2D(_GrassDepthTex, sceneUVZ.xy).r) + _ZBufferParamB );
			sceneUVZ.z=(sceneUVZ.z>_ZBufferFarClip)?10000:sceneUVZ.z;
			...
			float3 EyeDirTan = -normalize(IN.viewDir);
			...
			float3 rayPos_ground=float3(rayPos.xy + EyeDirTan.xy * delta.z, ((EyeDirTan.z<0) ? -GRASSDEPTH : 0));
					...
					half4 ground_col=tex2D(_MainTex, rayPos_ground.xy);
					ground_col.rgb*=_Color.rgb*2;
		 		#else
					half4 ground_col;
					ground_col.rgb=_bottom_coloring_value.rgb;
				#endif
	 			ground_col.w=(1-zoffset);
			#endif
			
			half4 _col, _blades_col;
			bool zhit=false;
			bool zhit_cl=false;
			bool xyhit;
			float2 _uv;
		 	...
		 	for(hitcount=0; hitcount < MAX_RAYDEPTH; hitcount++) {
				...
				bool xy_flag;
		 		...
		 		if ((delta.z<delta.x) && (delta.z<delta.y)) {
		 			...
		 			rayPos_tmp = xy_flag ? rayPos.xyz : rayPos.yxz;
		 		
		 			delta.xyz=xy_flag ? float3(delta_next.x, delta.yz-delta.x) : float3(delta.x-delta.y, delta_next.y, delta.z-delta.y);
		 		}
		 		
				#ifdef WIND
					float4 wind_tex_val=tex2D(NOISE_WIND_TEX, rayPos.xy*_wind_freq+_Time.xx*_wind_speed);
					float2 wind_offset=0;
					if (!zhit) {
						wind_offset=_wind_amp * (float2(wind_tex_val.NOISE_CHANNEL_WINDX, wind_tex_val.NOISE_CHANNEL_WINDY)-0.5);
						float dt=1+rayPos.z*GRASS_SLICE_NUM;
						wind_offset*=dt;
					}
					_uv=rayPos_tmp.yz+float2((xy_flag ? wind_offset.x : wind_offset.y),rayPos_tmp.x*PREMULT+hgt);
				#else
					_uv=rayPos_tmp.yz+float2(0,rayPos_tmp.x*PREMULT+hgt);
				#endif
		 		
			 	#ifdef HASH_SLICES
					#ifdef CUSTOM_HASH_FUNCTION
						_uv.x+=CUSTOM_HASH_FUNCTION;
			 		#else
						_uv.x+=tex2D(_NoiseTexHash, float2(rayPos_tmp.x+0.013,0));
			 		#endif
				#endif
				
				#ifdef DISTANCE_BLURING
			 		_blades_col=distanceS.z * tex2Dlod(_BladesTex, float4(_uv, 0, distanceS.x ));
			 		_blades_col+=distanceS.y * tex2Dlod(_BladesTex,float4(_uv, 0, distanceS.w )); // distanceS.w==distanceS.x+1
					float mipcut=clamp(-rayPos.z*70,0,1);
	 				_blades_col.a*=mipcut;
				#else
			 		_blades_col=tex2Dlod(_BladesTex, float4(_uv, 0, 0 ));
				#endif
	 			_blades_col.rgb*=_blades_col.a;
				#ifdef GLOWING
					#ifndef DISTANCE_BLURING
						float mipcut=clamp(-rayPos.z*70,0,1);
					#endif
					_blades_col+=_glowing_value*(tex2Dlod(_BladesTex,float4(_uv, 0, GLOWING_MIP_LEVEL )) - _blades_col)*mipcut;
				#endif
					
	 			#ifdef FADE_PARALLELS
		 			float fade_parallelsXY=xy_flag ? fade_parallels.x : fade_parallels.y;
		 			_blades_col*=fade_parallelsXY;	 			
	 			#endif
	 			
	 			_col=zhit ? _col : _blades_col;
	 			
	 			#ifdef FILL_GROUND
		 			#ifdef BOTTOM_COLORING
		 				cval=1+rayPos.z*bottom_cut;
		 				cval=clamp(cval,0,1);
	 					half3 ca=zhit ? _bottom_coloring_value.rgb : (_bottom_coloring_value.rgb*_col.a);
		 				_col.rgb=zhit_cl ? _col.rgb : (ca+cval*(_col.rgb - ca));
		 			#endif
		 		#else
		 			#ifdef BOTTOM_COLORING
		 				cval=1+rayPos.z*bottom_cut;
		 				...
		 				_col.rgb=zhit ? _col.rgb : (ca+cval*(_col.rgb - ca));
		 			#endif
		 		#endif
		 		
 				c=(rayPos.z>-0.001) ? c : (zhit_cl ? c : (c+(1-c.w)*_col));
 				zhit_cl=zhit;
			}
			#ifdef MBLUR
			...
			#endif
			#ifdef BORDER_FRAY
				c.w*=IN.params.z*(tex2D(NOISE_FRAY_TEX, rayPos.xy*_border_fray_tiling).NOISE_CHANNEL_FRAY-_border_fray_strength*2) + 1;		
			#endif
			base_col.rgb*=_Color.rgb*2;
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			int hitcount;
		 	for(hitcount=0; hitcount < MAX_RAYDEPTH; hitcount++) {
				...
				#ifdef WIND
					float4 wind_tex_val=tex2D(NOISE_WIND_TEX, rayPos.xy*_wind_freq+_Time*_wind_speed);
					float2 wind_offset=0;
					if (!zhit) {
						wind_offset=_wind_amp * (float2(wind_tex_val.NOISE_CHANNEL_WINDX, wind_tex_val.NOISE_CHANNEL_WINDY)-0.5);
						float dt=1+rayPos.z*GRASS_SLICE_NUM;
			...
			int hitcount;
		 	for(hitcount=0; hitcount < MAX_RAYDEPTH; hitcount++) {
				...
				bool xy_flag;
	 			...
	 			#ifdef MOW_ENABLED
	 				_blades_col = (rayPos.z<-GRASSDEPTH*auxtexval) ? _blades_col : half4(0,0,0,0);
	 				_blades_col.rgb*=(1+auxtexval*_auxtex_modulator_value);
	 			#endif
